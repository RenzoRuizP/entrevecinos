...
final class SoporteDashboard extends Conexion
{
     ...
     */
    /**
     * Contadores operativos mínimos para la campana global de Soporte.
     * Evita cargar la bandeja "Atender ahora" durante el sondeo periódico.
     */
    public function pendientesOperativos(): array
    {
        $cuentas = 0;
        if ($this->tableExists('usuario')) {
            $estadoUsuario = $this->firstColumn('usuario', ['estado']);
            if ($estadoUsuario) {
                $cuentas = $this->countSql("
                    SELECT COUNT(*)
                    FROM `usuario`
                    WHERE {$this->qc($estadoUsuario)} = 1
                ");
            }
        }

        $publicaciones = 0;
        if ($this->tableExists('producto')) {
            $visible = $this->firstColumn('producto', ['visible']);
            $estadoPublicacion = $this->firstColumn('producto', [
                'estado_revision',
                'estado_publicacion',
                'estado_moderacion',
                'estado',
            ...
            if ($visible) {
                $publicaciones = $this->countSql("
                    SELECT COUNT(*)
                    FROM `producto`
                    WHERE {$this->qc($visible)} = 1
                ");
            } elseif ($estadoPublicacion) {
                $publicaciones = $this->countEstadoValues('producto', $estadoPublicacion, [
                    'en_revision',
                    'revision',
                    'pendiente',
                    'pendiente_revision',
        ...
        $recargas = 0;
        if ($this->tableExists('recarga_saldo')) {
            $estadoRecarga = $this->firstColumn('recarga_saldo', ['estado', 'estado_recarga']);
            if ($estadoRecarga) {
                $expr = $this->estadoExpr($estadoRecarga);
                $recargas = $this->countSql("
                    SELECT COUNT(*)
                    FROM `recarga_saldo`
                    WHERE {$expr} IN ('pendiente', 'pendiente_validacion', 'en_revision')
                ");
            }
        }
        ...
        $resultado = [
            'cuentas' => max(0, $cuentas),
            'publicaciones' => max(0, $publicaciones),
            'recargas' => max(0, $recargas),
            'servicios' => max(0, $servicios),
        ];
