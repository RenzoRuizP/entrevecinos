...
(function () {
  ...
  function ensure() {
    ...
    const css = `
      ...
      #ev-sc-shell .ev-sc-proposal.is-expired{border-color:#E5E7EB;background:#F9FAFB;box-shadow:none;opacity:.86}
      #ev-sc-shell .ev-sc-proposal.is-expired h4,#ev-sc-shell .ev-sc-proposal.is-expired p{color:#6B7280}
      #ev-sc-shell .ev-sc-proposal.is-expired .ev-sc-proposal-state{border-color:#FED7AA;background:#FFF7ED;color:#C46B05}
  ...
  function quoteHtml(quote) {
    const quoteState = String(quote.estado || '');
    const isCurrent = ['vigente', 'aceptada', 'requiere_actualizacion'].includes(quoteState);
    const isExpired = quoteState === 'vencida';
    ...
    return `
      <article class="ev-sc-proposal ${isCurrent ? 'is-current' : ''} ${isExpired ? 'is-expired' : ''}">
  ...
  function conversationActions(data) {
    ...
    if (role === 'proveedor' && ['pendiente_proveedor', 'ajuste_solicitado', 'ajuste_cotizacion_solicitado', 'cotizacion_vencida'].includes(state)) {
      const textoBoton = ['ajuste_cotizacion_solicitado', 'cotizacion_vencida'].includes(state)
        ? 'Emitir nueva cotización final'
        : 'Emitir cotización final';
    ...
    if (role === 'solicitante' && state === 'cotizacion_vencida') {
      actions.push(`<button type="button" class="ev-sc-action orange" data-sc-action="renew-quote"><i class="bi bi-arrow-repeat"></i> Solicitar nueva cotización</button>`);
    }
    ...
    if (role === 'proveedor' && ['pendiente_proveedor', 'informacion_adicional_solicitada', 'ajuste_solicitado', 'ajuste_cotizacion_solicitado', 'cotizacion_final_enviada', 'cotizacion_vencida'].includes(state)) {
      actions.push(`<button type="button" class="ev-sc-action danger" data-sc-action="decline-provider"><i class="bi bi-slash-circle"></i> No continuar</button>`);
    }
  ...
  async function renewExpiredQuote() {
    if (accionEnCurso) return false;

    await ejecutarAccionConConfirmacion('Solicitar nueva cotización', {
      text: 'La cotización anterior venció y ya no puede aceptarse. Puedes pedir al proveedor una nueva versión para continuar la negociación.',
      confirmText: '<i class="bi bi-arrow-repeat"></i> Solicitar',
      cancelText: '<i class="bi bi-x-circle"></i> Cancelar'
    }, async (_resultado, codigoSolicitud) => {
      await request(
        `${BASE}/api/servicios/solicitudes/${codigoSolicitud}/solicitar-ajuste-cotizacion`,
        new URLSearchParams({ mensaje: 'La cotización anterior venció. Solicito una nueva cotización para continuar la coordinación.' })
      );
    });
  }

  async function declineProvider() {
    const options = structuredReasonDialogOptions({
      ...
      maxLength: 500,
      options: [
        ['disponibilidad', 'Ya no tengo disponibilidad para atender el servicio'],
        ['sin_acuerdo', 'No logramos llegar a un acuerdo'],
        ['alcance', 'El servicio solicitado está fuera de mi alcance'],
        ['fecha_horario', 'No puedo atender en la fecha u horario requeridos'],
        ['otro', 'Otro motivo']
      ]
    });

    await ejecutarAccionConConfirmacion('No continuar con la solicitud', options, async (resultado, codigoSolicitud) => {
      await request(
        `${BASE}/api/servicios/solicitudes/${codigoSolicitud}/rechazar`,
  ...
  function bindControls() {
    ...
    document.querySelector('[data-sc-action="renew-quote"]')?.addEventListener('click', () => renewExpiredQuote().catch((error) => toast('error', error.message)));
    document.querySelector('[data-sc-action="decline-provider"]')?.addEventListener('click', () => declineProvider().catch((error) => toast('error', error.message)));
