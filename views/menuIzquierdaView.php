...
if (!in_array($tipoConjuntoSidebar, ['urbanizacion', 'condominio'], true)) {
    $tipoConjuntoSidebar = '';
}
...
if ($nombreComunidadSidebar !== 'Tu comunidad' && $tipoConjuntoSidebar !== '') {
    ...
    if ($nombreLower !== $labelLower && !str_starts_with($nombreLower, $labelLower . ' ')) {
        $nombreComunidadSidebar = $labelComunidadSidebar . ' ' . $nombreComunidadSidebar;
    }
}
  ...
  <div id="evSidebarHomeExtras" class="ev-sidebar-footer" aria-label="Accesos secundarios">
    ...
    >
      <div class="ev-sidebar-community-icon" aria-hidden="true">
        <i
          class="<?= htmlspecialchars($iconoComunidadSidebar, ENT_QUOTES, 'UTF-8') ?>"
          id="evSidebarCommunityIcon"
        ></i>
      </div>
